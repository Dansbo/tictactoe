...
@cnt8	cmp #8			;If count is not 8 then
	...
	lda .keypress,x		;Load keypress number into A
	jmp @end_ai

@chk_nw
	ldy #9			;Prepare Y for byte counter
	lda #0
	sta .wincnt		;Reset match counter (.wincnt)
	lda #<.nw1
	sta TMP0
	lda #>.nw1
	sta TMP1
	jsr @chk_nw_plc		;See if we can find to matches to scenario
	cmp #2
	bne @nw2		;if not check next scenario
	lda .Occ_place +6	;Is tile 7 available?
	bne @nw2		;If not check next scenarios
	lda .keypress +6	;If available the go win
	jmp @end_ai		;Go to gameloop with tile choice

@nw2	lda #0
	sta .wincnt		;Reset match counter
	lda #<.nw2
	sta TMP0
	lda #>.nw2
	sta TMP1
...
Matching_nw
	...
	lda .wincnt
	cmp #2			;If .wincnt=3 we have a winner!
	rts
